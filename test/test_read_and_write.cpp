...
int main(int argc, char* argv[]) {
  ...
  Kokkos::initialize(argc, argv);
  {
    ...
    std::string output = "test_read_write.gmv";

    // read mesh then write it then reload it
    Mesh_Kokkos initial = io::read_mesh(input);
    io::write_mesh(initial, output);
    Mesh_Kokkos reloaded = io::read_mesh(output);
    ...
    for (int i = 0; i < reloaded.mirror_points_.extent(0); i++) {
      auto const& p = reloaded.mirror_points_(i);
    ...
    int const n = reloaded.mirror_cells_.extent(0);
    for (int i = 0; i < n; i++) {
      int const m = reloaded.mirror_num_verts_per_cell_(i);
      for (int j = 0; j < m; j++) {
        int const vij = reloaded.mirror_cells_(i, j, 0);
