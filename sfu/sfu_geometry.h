...
Point normVec(Point a, Point b) {
  // Direction vec
  float dx = b.x - a.x; // x2 - x1
  float dy = b.y - a.y; // y2 - y1
...
float dotProduct(Point const& v, Point const& n) {
  float const product = (v.x * n.x) + (v.y * n.y);

  float const epsilon = 1.e-15;
...
Point middle_point(Segment const& points) {
  float mx = (points.a.x + points.b.x) / 2;
  float my = (points.a.y + points.b.y) / 2;
