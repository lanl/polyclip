  ...
  if(material_format == 1){ // all cells clipped (small meshes)
	...
	if(num_clip == 0){
	   ...
	   for(int l = 0; l < n_lines; l++){
	      // Line Details
    	      float nx = clipped_part.mirror_line_(l).n.x;
    	      float ny = clipped_part.mirror_line_(l).n.y;
    	      float d = clipped_part.mirror_line_(l).d;

              int id = mesh.mirror_cells_(c, 0, 0);
	      auto const p = mesh.mirror_points_(id);
	      float side = nx * p.x + ny * p.y;
